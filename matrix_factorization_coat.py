...
class MF_KBIPS_Gau(nn.Module):
    ...
    def fit(self, x, y, y_ips=None, G=5, gamma = 1, C = 1e-5,
        ...
        self.W.load_state_dict(torch.load('data/coat/coat_user.pth'))
        self.H.load_state_dict(torch.load('data/coat/coat_item.pth'))
        ...
        for epoch in range(num_epoch):
            ...
            for idx in range(total_batch):
                ...
                # weight model 
                w = self.weight_model.forward(sub_x)
                w = w * inv_prop
...
class MF_AKBIPS_Gau(nn.Module):
    def __init__(self, num_users, num_items, embedding_k=4, *args, **kwargs):
        ...
        self.weight_model = MF_BaseModel(
            num_users = self.num_users, num_items = self.num_items,embedding_k=self.embedding_k, *args, **kwargs)  
        self.epsilon = torch.nn.Parameter(torch.rand(1,640))   # batchsize =128  G=5                        
        self.sigmoid = torch.nn.Sigmoid()
        self.xent_func = torch.nn.BCELoss()
        self.mse = torch.nn.MSELoss(reduction='mean')
    ...
    def fit(self, x, y, y_ips=None, G=5, gamma = 1, C = 1e-5,num_w_epo = 3,
...
class MF_WKBIPS_Gau(nn.Module):
    def __init__(self, num_users, num_items, embedding_k=4, *args, **kwargs):
        ...
        self.weight_model = MF_BaseModel(
            num_users = self.num_users, num_items = self.num_items,embedding_k=self.embedding_k, *args, **kwargs)
        self.epsilon = torch.nn.Parameter(torch.rand(1,640))  
        self.sigmoid = torch.nn.Sigmoid()
        self.xent_func = torch.nn.BCELoss()
        self.mse = torch.nn.MSELoss(reduction='mean')
    ...
    def fit(self, x, y, y_ips,gamma = 1,C=1e-2,num_w_epo = 3,J=3,
        ...
        for epoch in range(num_epoch):
            ...
            for idx in range(total_batch):
                ...
                xent_loss = F.binary_cross_entropy(pred, sub_y, weight=w, reduction="sum")         
                imputation_loss = F.binary_cross_entropy(pred, imputation_y, reduction="sum")                 
